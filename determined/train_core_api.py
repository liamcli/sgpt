...
parser.add_argument(
    "--wandbwatchlog", default="all", type=str
)  # Set e.g. to just gradients for large models
...
with det.core.init() as core_context:
    ...
    if not args.no_training:
        ...
        with gzip.open(hard_negatives_filepath, "rt") as fIn:
            for i, line in tqdm.tqdm(enumerate(fIn)):
                ...
                if (
                    args.train_dataset_max_size is not None
                    and i > args.train_dataset_max_size
                ):
                    break
    ...
    with open(collection_filepath, encoding="utf8") as fIn:
        for line in fIn:
            ...
            if (
                pid in needed_pids
                or args.dev_corpus_max_size <= 0
                or len(dev_corpus) <= args.dev_corpus_max_size
            ):
    ...
    if args.add_special_token or args.speca:
        ...
        word_embedding_model.bos_spec_token_q_rep = word_embedding_model.tokenizer.encode(
            "[", add_special_tokens=False
        )[0]
        word_embedding_model.eos_spec_token_q = word_embedding_model.tokenizer.encode(
            "]", add_special_tokens=False
        ...
        word_embedding_model.bos_spec_token_d_rep = word_embedding_model.tokenizer.encode(
            "{", add_special_tokens=False
        )[0]
        word_embedding_model.eos_spec_token_d = word_embedding_model.tokenizer.encode(
            "}", add_special_tokens=False
